...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#tab'
            // Servlet 返回一个json字符串
            ,url:'<%=request.getContextPath()%>/BookTypeServlet?action=query'
            ,toolbar: '#toolbarDemo'
            ,title: '图书分类表'
            ,cols: [[
            ...
            switch(obj.event){
                case 'deleteAll':
                    ...
                    layer.confirm('确认要删除这些信息吗？',function(index) {
                        $.ajax({
                            type: "post",
                            url: "<%=request.getContextPath()%>/BookTypeServlet",
                ...
                case 'toadd':
                    ...
                    layer.open({
                        ...
                        offset: 'auto', //右下角弹出
                        anim: 2,
                        content: ['typeadd.jsp', 'no'], //iframe的url，no代表不显示滚动条
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                layer.open({
                    ...
                    offset: 'auto', //右下角弹出
                    anim: 2,
                    content: ['<%=request.getContextPath()%>/BookTypeServlet?action=queryOne&id='+data.typeId, 'no'], //iframe的url，no代表不显示滚动条
                    end: function(){ //此处用于演示 关闭之后执行
                    }
                });
            }
